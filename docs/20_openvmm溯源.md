# 31 openvmm/WSL “为什么这么写”溯源：git 历史实证 + 版本缺陷区间重大修正
...
> 背景：20 号文档已把“机制路径”钉到源码行号；本文回答“为什么这样写”，并对版本缺陷区间的两处错误归因做勘误（#40187 归属、#41125/#41051 首发版本）。
...
4. **【重大勘误】2.7.8.0 的 guest 侧 tracker 已含 #40187**（inline 端口 0 解析）：#40187（3f00f988，04-22）修改的是 `src/linux/init/GnsPortTracker.cpp`（guest 侧，随 WSL 应用发布），2.7.8 tag（06-05）经 merge-base 验证包含。此前 27/28 号“本机 wsldevicehost.dll（04-20 构建）不含 #40187”是**类别错误**——#40187 不在 DLL 里，DLL 构建日期只约束 host 侧 glue（ConsommeNetworking + net_consomme）。
5. **与 ETW 证据（30 号：host 侧 0 个 port request 事件）直接对应的缺失修复是 #41051（PIDFD_THREAD）**：2.7.8 的 DuplicateSocketFd 用 `pidfd_open(Pid, 0)`，非主线程（tid ≠ pid）bind(0) 时 pidfd_open 失败 → 注册请求从未发出 → host 0 事件。我们的 Go 探针 bind 跑在 Go runtime 非主线程上，机制链条完全自洽。
6. **发布线勘误**：#40287 首发 2.8.5+（05-13）；#41125（listen autobind）与 #41051（PIDFD_THREAD）首发 **2.9.5（2026-08-11）**；2.7.x 全系（至 2.7.12，08-17）与 2.8.x 均不含。27 号“#41125 从 2.7.11 起”表述错误。
...
- `create_bound_socket`（net_consomme/src/lib.rs）是 4a73bb30（#3364，2026-04-29，openvmm CLI hostfwd）才引入，**晚于**本机 DLL 锚点 0bb5cf75（04-19）——本机 DLL 无此函数，20 号文档引用的 lib.rs 行号需注意版本。
...
特征：每个提交正文都点名具体报告场景与用户影响；修复只覆盖该场景，未覆盖的并发/退化路径仍出问题——与 26/28/29 号文档的 churn/窗口式观测一致。
...
- 2.7.8.0 的端口 0 处理顺序（tag 2.7.8 代码实证）：`TrackPort`（显式端口）→ `CompleteRequest`（放行 bind，返回 0）→ `ResolvePortZeroBind` inline → `HandleRequest`（同步事务到 host）。bind 返回 0 与注册完成是两件事——与 28 号 E 实验（立即自连 0%）自洽。
- 因此 27 号表格“不含 #40187”、28 号“仍使用 dup socket 异步解析”、29 号“#40187 修复前实现的竞态”等表述**全部作废**，以本文为准。
...
- 对应链条：Go 探针的 bind() 是阻塞 syscall，跑在 Go runtime 非主线程（tid ≠ pid）→ pidfd_open 失败 → ResolvePortZeroBind 无结果 → HandleRequest 从未调用 → host 侧 0 个 port request 事件（30 号 ETW）→ 自连被 RST（10061）。**机制与官方修复方向完全吻合**。
...
- 27 号“#41125（2.7.11 起）”错误；2.7.11/2.7.12 不含这两个修复。

## 五、对既有文档的勘误清单

| 文档 | 需修正处 | 修正 |
| --- | --- | --- |
| 27 号 | 第一节表格（#40187 归属）、“2.7.8.0 不含 #40187”、“#41125 从 2.7.11 起” | #40187 含；#41125/#41051 首发 2.9.5 |
| 28 号 | 结论 1/2、“与官方问题的对应”第 2 条 | 缺失修复 = #41051（非 #40187） |
| 29 号 | “机制推断”段（#40187 前提） | 同上 |
| 30 号 | 第五节结论 4 前提 | 不冲突；版本对应修正为 #41051 |
| 面试详案 | E11 行、7.1 第 4 条、8.3 第 6 条 | 已同步修正（08-19） |

各文档已追加“勘误（31 号）”节，历史正文保留不改写（研究链可追溯）。

## 六、对投稿叙事的影响

- “2.7.8.0 处于官方已知缺陷区间”结论**不变**，锚点后移：缺 #41051（线程 bind 追踪）+ #41125（listen 隐式绑定），#40187 已在。
- 新增最强对应：**ETW“host 0 事件”= #41051 所修 bug 的直接实证签名**；#41051 由官方确认（#41039 场景）并于 2026-08-05 合并、2.9.5（08-11）发布——均早于我们评论 v4 发出（08-18）。
- 建议叙事（若更新评论）：“健康态下非主线程 bind(0) 的注册请求从未到达 host（host 侧 0 事件），与后续版本对线程 bind 跟踪的修复（#41051 语义）一致；本机 2.7.8.0 处于该修复发布之前的缺陷区间。”注意外部编号禁令：对外文本不提 PR 编号，只描述语义。
- 升级验证是低成本闭环动作：本机装 2.9.5+ 后重跑 port0_*.go 探针，若成功率恢复 ~100% 即闭环（需用户拍板，见 39 号预案之外的新增选项）。
...
> 详细证据见 36 号文档；触发原因：2.9.5/2.9.6/2.9.7 无安装资产（GitHub release API 实证仅源码 zip/tar.gz），#71 运行时验证不可执行，转源码级验证（只读）。

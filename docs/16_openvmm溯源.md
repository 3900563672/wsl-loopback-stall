...
> 材料：openvmm（本地 blobless clone，HEAD eef8b22e）、microsoft/WSL（本地 clone，HEAD 1e14a49b）
...
- 新增最强对应：**ETW“host 0 事件”= #41051 所修 bug 的直接实证签名**；#41051 由官方确认（#41039 场景）并于 2026-08-05 合并、2.9.5（08-11）发布——均早于本案例正式提交（08-18）。
- 结论表述：“健康态下非主线程 bind(0) 的注册请求从未到达 host（host 侧 0 事件），与后续版本对线程 bind 跟踪的修复（#41051 语义）一致；本机 2.7.8.0 处于该修复发布之前的缺陷区间。”
- 升级验证是低成本闭环动作：本机装 2.9.5+ 后重跑 port0_*.go 探针，若成功率恢复 ~100% 即闭环（见 10 号升级验证）。

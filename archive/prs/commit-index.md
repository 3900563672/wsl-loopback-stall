...
> 迁移自早期研究工作区，经 `git filter-repo` 按路径提取，作者/日期/消息完整保留，哈希已重写。共 23 个 commit，按时间顺序（旧→新）：
...
49000f2 chore: 清洗公开内容——本地路径/IP/MAC 泛化、内部过程措辞中性化、移除观察脚本与子集日志包
66f35a9 docs: 重生成 commit 索引（对齐重写后历史）
d748eae docs: 重生成 commit 索引（对齐重写后历史）
